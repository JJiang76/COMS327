...
void io_display(dungeon_t *d)
{
  ...
  if (d->fog) {
    if ((c = io_nearest_visible_monster(d))) {
      if (pcy >= c->position[dim_y] - 2 && pcy <= c->position[dim_y] + 2
        && pcx >= c->position[dim_x] - 2 && pcx <= c->position[dim_x] + 2) {
        ...
        mvprintw(22, 55, "You sense something near..");
