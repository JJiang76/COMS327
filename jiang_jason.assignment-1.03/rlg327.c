...
typedef struct dungeon {
  ...
  uint32_t tunnel_map[DUNGEON_Y][DUNGEON_X];
  uint32_t non_tunnel_map[DUNGEON_Y][DUNGEON_X];
  pair_t pc;
} dungeon_t;
...
static void dijkstra_nontunnel(dungeon_t *d)
{
  ...
  while ((p = heap_remove_min(&h))) {
    ...
    for (i = p->pos[dim_y] - 1; i <= p->pos[dim_y] + 1; i++) {
      for (j = p->pos[dim_x] - 1; j <= p->pos[dim_x] + 1; j++) {
        if (!(i == p->pos[dim_y] && j == p->pos[dim_x])) {
          if ((path[i][j].hn) && (path[i][j].cost > p->cost + (1 + (hardnesspair(p->pos) / 85)))) {
            path[i][j].cost = p->cost + (1 + (hardnesspair(p->pos) / 85));
  ...
  for (y = 0; y < DUNGEON_Y; y++) {
    for (x = 0; x < DUNGEON_X; x++) {
      d->non_tunnel_map[y][x] = path[y][x].cost;
    }
  }
...
static void dijkstra_tunnel(dungeon_t *d)
{
  ...
  for (y = 0; y < DUNGEON_Y; y++) {
    for (x = 0; x < DUNGEON_X; x++) {
      if (mapxy(x, y) != ter_wall_immutable && mapxy(x, y)) {
        path[y][x].hn = heap_insert(&h, &path[y][x]);
      } else {
        path[y][x].hn = NULL;
      }
  ...
  for (y = 0; y < DUNGEON_Y; y++) {
    for (x = 0; x < DUNGEON_X; x++) {
      d->tunnel_map[y][x] = path[y][x].cost;
    }
  }
...
void render_non_tunnel_map(dungeon_t *d) {
  int x, y;
  ...
  for (y = 0; y < DUNGEON_Y; y++) {
    for (x = 0; x < DUNGEON_X; x++) {
      if (y == d->pc[dim_y] && x == d->pc[dim_x]) {
        ...
        if (d->non_tunnel_map[y][x] != INT_MAX) {
          printf("%d", (d->non_tunnel_map[y][x] % 10));
        }
        else {
          printf(" ");
        }
...
void render_tunnel_map(dungeon_t *d) {
  int x, y;
