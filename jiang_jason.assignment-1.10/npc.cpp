...
npc::npc(dungeon *d, monster_description &m) : md(m)
{
  ...
  if (d->floor_num > 0) {
    dice spd_mod((2 * f) + (int)(m.speed.get_base() * (1 + .2*f)), 
                     (int)(m.speed.get_number() + f/2),
                 (2 * f) + (int)(m.speed.get_sides() * (1 + .3*f)));

    dice hp_mod((5*f) + (int)(m.hitpoints.get_base() * (1 + .2*f)),
                    (int)(m.hitpoints.get_number() + f/2),
                (5*f) + (int)(m.hitpoints.get_sides() * (1 + .2f)));

    dice *att_mod = new dice((3*f) + (int)(m.damage.get_base() * (1 + .2*f)), 
                                 (int)(m.damage.get_number() + f/2),
                             (3*f) + (int)(m.damage.get_sides() * (1 + .4*f)));
