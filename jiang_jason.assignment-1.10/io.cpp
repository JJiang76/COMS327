    ...
    for (pos[dim_x] = -PC_VISUAL_RANGE;
      ...
      if (cursor[dim_y] == d->PC->position[dim_y] + pos[dim_y] &&
        ...
        case ter_cage_door:
          attron(COLOR_PAIR(COLOR_RED));
          mvaddch(d->PC->position[dim_y] + pos[dim_y] + 1,
                  d->PC->position[dim_x] + pos[dim_x], 'H');
          attroff(COLOR_PAIR(COLOR_RED));
          break;  
        case ter_princess:
          attron(COLOR_PAIR(COLOR_YELLOW));
          mvaddch(d->PC->position[dim_y] + pos[dim_y] + 1,
                  d->PC->position[dim_x] + pos[dim_x], 'B');
...
void io_display(dungeon *d)
{
  ...
  for (visible_monsters = -1, pos[dim_y] = 0;
    ...
    for (pos[dim_x] = 0; pos[dim_x] < DUNGEON_X; pos[dim_x]++) {
      ...
      if (d->character_map[pos[dim_y]]
        ...
        case ter_cage_door:
          attron(COLOR_PAIR(COLOR_RED));
          mvaddch(pos[dim_y] + 1, pos[dim_x], 'H');
          attroff(COLOR_PAIR(COLOR_RED));
          break; 
        case ter_princess:
          attron(COLOR_PAIR(COLOR_YELLOW));
          mvaddch(pos[dim_y] + 1, pos[dim_x], 'B');
          attroff(COLOR_PAIR(COLOR_YELLOW));
          break;
        default:
 /* Use zero as an error symbol, since it stands out somewhat, and it's *
  * not otherwise used.                                                 */
  ...
  mvprintw(23, 1, "Floor %d, Pos:(%2d,%2d)", d->floor_num, 
           d->PC->position[dim_x], d->PC->position[dim_y]);
  mvprintw(22, 1, "%sLVL:%d | HP:%d | STR:%d | DEF: %d | SPD:%d | DEX:%d | LK:%d | EXP:%d/%d", 
                    d->PC->exp == d->PC->exp_to_next() ? "(!)" : "   ",
                    d->PC->level, d->PC->hp, d->PC->stats[strength], d->PC->stats[defense],
                    d->PC->speed, d->PC->stats[dex], d->PC->stats[luck], d->PC->exp, d->PC->exp_to_next());
  mvprintw(23, 30, "Nearest visible monster: ");
  if ((c = io_nearest_visible_monster(d))) {
    attron(COLOR_PAIR(COLOR_RED));
    mvprintw(23, 55, "%c at %d %c by %d %c.",
             c->symbol,
...
uint32_t io_teleport_pc(dungeon *d)
{
  ...
  do {
    ...
    case ter_cage_door:
      attron(COLOR_PAIR(COLOR_RED));
      mvaddch(dest[dim_y] + 1, dest[dim_x], 'H');
      attroff(COLOR_PAIR(COLOR_RED));
      break;  
    case ter_princess:
      attron(COLOR_PAIR(COLOR_YELLOW));
      mvaddch(dest[dim_y] + 1, dest[dim_x], 'B');
      attroff(COLOR_PAIR(COLOR_YELLOW));
      break;
    default:
 /* Use zero as an error symbol, since it stands out somewhat, and it's *
...
uint32_t io_inspect_top_floor(dungeon *d)
{
  uint32_t n;
  pair_t dest;
  int c;
  ...
  mvprintw(0, 0,
           "Choose a character.  'g' or '.' to inspect; ESC to cancel.");
  ...
  * not otherwise used.                                                 */
    ...
    case KEY_LEFT:
      ...
      break;
            ...
            && mappair(dest) != ter_princess) 
            || (c != 'g' && c != '.' && c != 27 /* ESC */));


  if (mappair(dest) == ter_princess) {
    std::vector<monster_description>::iterator it;

    for (it = d->monster_descriptions.begin(); 
      it != d->monster_descriptions.end(); ++it) 
    {
      if (it->get_abilities() & NPC_PRINCESS) {
        break;
      }
    }

    snprintf(s, 80, "%-60s", it->get_name());

    for (n = 0, p = it->get_description(); *p; p++) {
      if (*p == '\n') {
        n++;
      }
    }
    ...
    mvprintw(0, 0, s);
    mvprintw(2, 0, it->get_description());  
  }
...
uint32_t io_display_level(dungeon *d) {
  uint32_t key, pts, n;
  bool b;

  d->PC->level_up();
  pts = d->PC->stat_points();

  mvprintw(5, 25, "%-25s");
  mvprintw(5, 25, " Level Up! Level %d -> %d", d->PC->level - 1, d->PC->level);
  mvprintw(6, 25, "%-25s", "");

  while (pts) {
    for (int i = 0; i < num_stats; ++i) {
        mvprintw(i + 7, 25, "  (%d) %-8s[%d]", i, stat_names[i], d->PC->stats[i]);
    }
    mvprintw(12, 25, "%-25s", "");
    mvprintw(13, 25, " Level up %d %s", pts, pts == 1 ? "stat" : "stats");
    mvprintw(14, 25, "%-50s", "");
    ...
    while (b) {
      switch (key = getch()) {
        ...
        case '4':
          ...
          break;
        default:
          mvprintw(14, 25, "Invalid input. Enter 0-4 to level up a stat");
      }
    }
  ...
  mvprintw(13, 25, "Press any key to continue");
  getch();
...
void io_display_ch(dungeon *d) {
  ...
  mvprintw(0, 0, "%s\'s stats:", d->PC->name);
  mvprintw(2, 0, "Killed: %d", d->PC->kills[kill_direct]);
  mvprintw(3, 0, "Avenged: %d", d->PC->kills[kill_avenged]);
  mvprintw(4, 0, "Types killed: %d", d->PC->kills[num_kill_types]);

  for (int i = 0; i < num_stats; ++i) {
    mvprintw(i + 6, 0, "%s: %d", stat_names[i], d->PC->stats[i]);
  }

  mvprintw(11, 0, "Hit: %d", d->PC->calc_hit());
  mvprintw(12, 0, "Dodge: %d", d->PC->calc_dodge());
  mvprintw(13, 0, "Crit: %d", d->PC->calc_crit());
...
uint32_t io_tele_floor(dungeon *d) {
  bool b = true;
  int32_t f;
  char key;
  ...
  mvprintw(3, 18, "Which floor to teleport to?");

  for (int i = 0; i < 10; ++i) {
    mvprintw(i + 5, 25, "[%d] Floor %d", i, i);
  }
  mvprintw(15, 25, "[a] Floor 10");
  ...
  while (b) {
    switch(key = getch()) {
      ...
      case 'F':
        return 0;
      default:
        mvprintw(17, 18, "Invalid input. Enter 0-9 or a or ESC/F to exit");
    }
  }
  ...
  do {
    ...
    case 'U':
      if (d->PC->exp >= d->PC->exp_to_next()) {
        ...
        io_queue_message("Not enough exp!");
      }
      fail_code = 1;
      break;
    case 'F':
