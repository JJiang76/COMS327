...
void place_pc(dungeon *d, int x, int y)
{
...
uint32_t pc::stat_points() {
  return 3 + (level / 5);
}

uint32_t pc::exp_to_next() {
  return level * 5;
}
...
uint32_t pc::calc_dmg() {
  uint32_t i, atk;

  for (i = atk = 0; i < num_eq_slots; i++) {
    if (i == eq_slot_weapon && !eq[i]) {
      atk += damage->roll();
    } 
    else if (eq[i]) {
      atk += eq[i]->roll_dice();
...
uint32_t pc::calc_def() {
    uint32_t i, def;

  for (i = def = 0; i < num_eq_slots; i++) {
    if (eq[i]) {
      def += eq[i]->get_defence();
    }
  }
...
uint32_t pc::calc_hit() {
  uint32_t i, hit;

  for (i = 0, hit = 0; i < num_eq_slots; i++) {
    if (eq[i]) {
      hit += eq[i]->get_hit();
    }
  }

  hit += stats[dex]/2 + stats[speed] / 3;

  return (hit >= 100 ? 100 : hit);
}

uint32_t pc::calc_dodge() {
  return (10 + (stats[dex] / 2) >= 100 ? 100 : 10 + (stats[dex] / 2));
}

uint32_t pc::calc_crit() {
  return (2 + (stats[luck] / 2) >= 100 ? 100 : 2 + (stats[luck] / 2));
}
