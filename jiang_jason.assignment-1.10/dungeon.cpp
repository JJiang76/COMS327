...
void gen_top_floor(dungeon *d) {

  for (int i = DUNGEON_X/2, j = DUNGEON_Y - 2; j > 10; --j) {
    mapxy(i, j) = ter_floor_hall;
  }
  mapxy(DUNGEON_X/2, DUNGEON_Y - 2) = ter_stairs_down;

  place_pc(d, DUNGEON_X/2, DUNGEON_Y - 2);
  d->character_map[d->PC->position[dim_y]][d->PC->position[dim_x]] = d->PC;

  for (int i = 25; i <= 55; ++i) {
    for (int j = 2; j <= 10; ++j) {
      mapxy(i, j) = ter_floor_room;
    }
  }

  mapxy(40, 4) = ter_princess;

  for (int i = 2; i <= 6; i++) {
    for (int j = 35; j <= 45; j+=10) {
      mapxy(j, i) = ter_cage_vert;
    }
  }

  for (int i = 35; i <= 45; ++i) {
    mapxy(i, 6) = ter_cage_hor;
  }

  mapxy(40, 6) = ter_cage_door;
