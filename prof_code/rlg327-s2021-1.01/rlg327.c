...
int main(int argc, char *argv[])
{
  ...
  if (swload) {   //load
    ...
    d.rooms = malloc(num_rooms * sizeof(room_t));

    int k;
    for (k = 0; k < num_rooms; k++) {
      fread(&d.rooms[k].position[dim_x], 1, 1, f);
      fread(&d.rooms[k].position[dim_y], 1, 1, f);
      fread(&d.rooms[k].size[dim_x], 1, 1, f);
      fread(&d.rooms[k].size[dim_y], 1, 1, f);
    }
